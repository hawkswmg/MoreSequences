...
def run_test_generate_points_on_circle():
    ...
    # Test 2
    expected = [rg.Point(205, 200),
                rg.Point(155, 250),
                rg.Point(105, 200),
                rg.Point(155, 150)]
    circle = rg.Circle(rg.Point(155, 200), 50)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4: Why '
    window = rg.RoseWindow(300, 300, title)
    circle = rg.Circle(rg.Point(150, 120), 100)
    circle.fill_color = 'black'
    draw_points_on_circle(window, circle, 60, 'blue')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        tiny_circle = rg.Circle(points[k],10)
        tiny_circle.fill_color = color
        tiny_circle.attach_to(window)
        dot = points[k]
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  4 slices, thin (thickness=5) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(220, 180), 120)
    circle.outline_thickness = 3
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3:  4 segments with black lines'
    window = rg.RoseWindow(550, 500, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    point1 = points[0]
    for k in range(number_of_segments - 2):
        line_start = rg.Line(point1, points[1])
        line_start.color = color
        line_start.thickness = thickness
        line_start.attach_to(window)
        lines = rg.Line(points[k + 1], points[k+2])
        lines.color = color
        lines.thickness = thickness
        lines.attach_to(window)
        lines_end = rg.Line(points[len(points)-1], point1)
        lines_end.color = color
        lines_end.thickness = thickness
        lines_end.attach_to(window)
        window.render()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    point1 = points[0]
    for k in range(number_of_lines - 2):
        line_start = rg.Line(point1, points[hops_to_next_point])
        line_start.color = color
        line_start.thickness = thickness
        line_start.attach_to(window)
        if k + 2 * hops_to_next_point:
            distance = (k + 2 * hops_to_next_point) - number_of_lines
        lines = rg.Line(points[k + hops_to_next_point], points[distance])
        lines.color = color
        lines.thickness = thickness
        lines.attach_to(window)
        lines_end = rg.Line(points[len(points) - 1], point1)
